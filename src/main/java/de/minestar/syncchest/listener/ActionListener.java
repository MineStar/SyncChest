...
public class ActionListener implements Listener {
	...
	public ActionListener(DataNode data, DatabaseHandler databaseManager) {
		this.data = data;
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (ChestUtils.getSyncChest(this.data, new BlockVector(event.getBlock()
				.getLocation())) != null) {
			PlayerUtils.sendError(event.getPlayer(), Core.NAME,
					"You cannot break this chest.");
			PlayerUtils.sendInfo(event.getPlayer(), Core.NAME,
					"This chest is a SyncChest. Desync it first.");
	...
	public void onEntityExplode(EntityExplodeEvent event) {
		...
		for (Block block : event.blockList()) {
			...
			if (ChestUtils.getSyncChest(this.data,
					new BlockVector(block.getLocation())) != null) {
				event.setYield(0.0f);
				event.setCancelled(true);
			}
		}
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if (ChestUtils.getSyncChest(this.data, new BlockVector(event.getBlock()
			...
			PlayerUtils.sendInfo(event.getPlayer(), Core.NAME,
					"The adjacent chest is a SyncChest. Desync it first.");
			event.setCancelled(true);
		}
	}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (playerState == PlayerState.NONE) {
			if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
				SyncChest syncChest = ChestUtils.getSyncChest(this.data,
						new BlockVector(event.getClickedBlock().getLocation()));
				if (syncChest != null) {
					// check permissions
					if (Permissions.hasPermission(event.getPlayer(),
							Permissions.CHEST_USE)) {
						this.data.setActiveSyncChest(event.getPlayer()
								.getName(), syncChest);
					} else {
						// cancel event
						event.setUseInteractedBlock(Event.Result.DENY);
						...
						// print error
						PlayerUtils.sendError(event.getPlayer(), Core.NAME,
								"You are not allowed to use SyncChests.");
					}
				} else {
					this.data.setActiveSyncChest(event.getPlayer().getName(),
							null);
				}
			}
			return;
		...
		Chest doubleChest = ChestUtils.getDoubleChest(location.getWorld(),
		...
		case CHEST_ADD: {
			...
			if (!Permissions.hasPermission(player, Permissions.CHEST_ADD)) {
				PlayerUtils.sendError(player, Core.NAME,
						"You are not allowed to create SyncChests.");
				return;
			}
			...
			if (syncChest != null) {
				PlayerUtils.sendError(player, Core.NAME,
						"This chest is already synced by '"
								+ syncChest.getParent().getParent()
										.getPlayerName() + "'.");
				return;
			}
			// check DoubleChest
		...
		case CHEST_REMOVE: {
			...
			if (!Permissions.hasPermission(player, Permissions.CHEST_REMOVE)) {
				PlayerUtils.sendError(player, Core.NAME,
						"You are not allowed to remove SyncChests.");
				return;
			}
		...
		case CHEST_INFO: {
			...
			if (!Permissions.hasPermission(player, Permissions.CHEST_INFO)) {
				PlayerUtils
						.sendError(player, Core.NAME,
								"You are not allowed to get informations about SyncChests.");
				return;
			}
			...
			if (syncChest == null) {
				// check for a DoubleChest
				if (!isDoubleChest) {
					PlayerUtils.sendInfo(player, Core.NAME,
							"This chest is not synced yet.");
					return;
				}

				// check DoubleChest
				syncChest = this.data.getChest(new BlockVector(doubleChest
						.getLocation()));
				if (syncChest == null) {
					PlayerUtils.sendInfo(player, Core.NAME,
							"This chest is not synced yet.");
				...
				// print info
				PlayerUtils.sendInfo(player, Core.NAME,
						"This chest is synced by '"
								+ syncChest.getParent().getParent()
										.getPlayerName() + "' in node '"
								+ syncChest.getParent().getNodeName() + "'.");
				return;
			}

			// print info
			PlayerUtils.sendInfo(player, Core.NAME, "This chest is synced by '"
					+ syncChest.getParent().getParent().getPlayerName()
					+ "' in node '" + syncChest.getParent().getNodeName()
					+ "'.");
			return;
		}
		case NONE:
	...
	private void tryToAddSyncChest(Player player, Location location,
			boolean doubleChest) {
		BlockVector position = new BlockVector(location);
		...
		if (this.data.hasSyncNode(player.getName(), nodeName)) {
			...
			for (int i = 0; i < chest.getInventory().getSize(); i++) {
				...
				if (itemStack != null
						&& itemStack.getTypeId() != Material.AIR.getId()) {
		...
		SyncChest syncChest = this.data.addChest(player.getName(), nodeName,
				position, doubleChest);
		...
		if (this.data.getSyncNode(player.getName(), nodeName).isDoubleChest() != doubleChest) {
			if (this.data.getSyncNode(player.getName(), nodeName)
					.isDoubleChest()) {
				PlayerUtils.sendError(player, Core.NAME,
						"All chests in this nodes must be double chests!");
			} else {
				PlayerUtils.sendError(player, Core.NAME,
						"All chests in this nodes must be single chests!");
			}
		...
		if (syncChest == null) {
			PlayerUtils.sendError(player, Core.NAME,
					"Could not create SyncChest! [0x00]");
			this.data.removeChest(position);
			this.nodeNames.remove(player.getName());
			this.data.getSyncNode(player.getName(), nodeName).setLocked(false);
			return;
		...
		if (!this.data.getSyncNode(player.getName(), nodeName).doStartUp()) {
			PlayerUtils.sendError(player, Core.NAME,
					"Could not synchronize inventories. [0x02]");
			this.data.removeChest(position);
			this.nodeNames.remove(player.getName());
			this.data.getSyncNode(player.getName(), nodeName).setLocked(false);
			return;
		...
		// print info
		PlayerUtils.sendSuccess(
				player,
				Core.NAME,
				"This chest is now synced in node '"
						+ this.nodeNames.get(player.getName()) + "'.");
		this.nodeNames.remove(player.getName());
	}

	private boolean tryToRemoveSyncChest(Player player, SyncChest syncChest) {
		// check owner
		if (!syncChest.isOwner(player.getName())
				&& !Permissions.hasPermission(player, Permissions.ADMIN)) {
			PlayerUtils.sendError(player, Core.NAME,
					"You are not allowed to remove this SyncChest.");
			return false;
		}
		...
		if (!this.databaseManager.deleteSyncChest(syncChest)) {
			PlayerUtils.sendError(player, Core.NAME,
					"Could not remove SyncChest from Database! [1x00]");
			return false;
		}
		...
		// print info
		PlayerUtils.sendSuccess(player, Core.NAME,
				"This chest is no longer a SyncChest.");
		return true;
	}
}
